...
Unzip or clone that root anywhere on the recipient machine, then add the root directory as a Codex local marketplace. Install `NGS Analysis` from the `NGS Analysis Local Plugins` marketplace. After installation, invoke it by asking for NGS routing or by referencing skills such as `ngs-analysis-router`, `ngs-fastq-qc`, `ngs-dna-somatic-variants`, or `ngs-scrna-seq`.
